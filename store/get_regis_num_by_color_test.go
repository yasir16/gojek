...
var _ = Describe("get regis number by color parking store test", func() {
	var (
		connection Store
	)
	connection = NewStore()
	...
	Context("leave store execute", func() {
		...
		It("leave help", func() {
			...
			res, err := connection.GetRegNumByColor().Execute(cmd)
			Ω(err).ShouldNot(HaveOccurred())
			Expect(res).To(Equal(schema.CMDRegisNumForCarsWithColorHint))
		})
		...
		It("get regis num by color success ", func() {
			cmd := &schema.Command{
				Command:   "registration_numbers_for_cars_with_colour",
		...
		It("get regis num by color not success ", func() {
			cmd := &schema.Command{
				Command:   "registration_numbers_for_cars_with_colour",
				Arguments: []string{"White"},
			}
